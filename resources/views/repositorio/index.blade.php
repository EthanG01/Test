                        ...
                        <div class="table-responsive">
                            <table class="table table-striped mt-2">
                                ...
                                @can('borrar-usuario')
                                                ...
                                                @if ($repositorio->estado == 0)
                                                    <td>
                                                        <a class="btn btn-sm btn-success"
                                                            href="{{ route('repositorios.estado', $repositorio->id) }}">Activo</a>
                                                    </td>
                                                @endif
                                                @if ($repositorio->estado == 1)
                                                    <td>
                                                        <a class="btn btn-sm btn-danger"
                                                            href="{{ route('repositorios.estado', $repositorio->id) }}">Inactivo</a>
                                    ...
                                    @can('ver-contenido')
                                    @if($esTrabajador)
                                    @foreach ($repU as $repo)
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            <td>{{ $repo->Autor->nombreAutor }} {{ $repo->Autor->apellidoAutor1 }}</td>
                                            <td>{{ $repo->nombre }}</td>
                                            <td>{{ $repo->fecha }}</td>
                                            <td>{{ $repo->tipo }}</td>
                                            <td><a href="/repositorio/{{ $repo->documento }}" target="black_">Ver
                                                    documento </a></td>
                                            @can('habilitar-repositorio')
                                                @if ($repo->estado == 0)
                                                    <td>
                                                        <a class="btn btn-sm btn-success"
                                                            href="{{ route('repositorios.estado', $repo->id) }}">Habilitar</a>
                                                    </td>
                                                @endif
                                                @if ($repo->estado == 1)
                                                    <td>
                                                        <a class="btn btn-sm btn-danger"
                                                            href="{{ route('repositorios.estado', $repo->id) }}">Desabilitar</a>
                                            ...
                                            <td colspan="3">
                                                <form action="{{ route('repositorioNuevo.destroy', $repo->id) }}"
                                                    class="formulario-eliminar" method="POST">
                                                    <a class="btn btn-sm btn-primary "
                                                        href="{{ route('repositorioNuevo.show', $repo->id) }}"><i
                                                            class="fa fa-fw fa-eye"></i> Mostrar</a>
                                                    @can('editar-repositorio')
                                                        <a class="btn btn-sm btn-success"
                                                            href="{{ route('repositorioNuevo.edit', $repo->id) }}"><i
                                                                class="fa fa-fw fa-edit"></i> Editar</a>
                                                    @endcan
                                                    @csrf
                                                    @method('DELETE')
